...
extension String {
    ...
    func qrCodeImage() -> UIImage? {
        return QRCodeDataSet(logo: UIImage(named: "ic_logo_min")!, url: self).getQRImage()
    }
}
...
struct QRCodeDataSet {
    ...
    //rgb(255,  , 118)
    init(logo: UIImage? = nil, url: String, backgroundColor: UIColor, color: UIColor, size: CGSize) {
        self.logo = logo
        self.url = url
